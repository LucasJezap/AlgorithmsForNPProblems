...
string solve(const Problem &p) {
    ...
    vector<Node *> fringe;
    make_heap(fringe.begin(), fringe.end(), uniform_comparator());

    fringe.push_back(initial);
    push_heap(fringe.begin(), fringe.end(), uniform_comparator());
    while (!fringe.empty()) {
        Node *node = fringe.front();
        pop_heap(fringe.begin(), fringe.end(), uniform_comparator());
        fringe.pop_back();
        if (p.goalTest(node->state))
            return get_solution(node);
        if (!p.deadlockTest(*node->state)) {
